...
- `assets/js/site.js` handles the header, overlay menu, map tooltips, count-ups, the portfolio search, and the news filters. `assets/js/map.js` is the zoomable map.
...
- The home hero is one still photograph: `assets/img/southold.jpg` (Southold, New York), set inline on
  the `.pan` div in `index.html`, with the caption in the `.vid-credit` div just below it. To change the
  photo, edit those two places; nothing else is involved. The `.shade` gradient in `site.css` sits over
  it and is deliberately heavy (.78/.62/.92) so the headline reads and the photo works as texture rather
  than subject. There is no hero JavaScript and no motion.
- Earlier versions rotated three Mixkit stock clips (`assets/video/`, still on disk, no longer
  referenced) and then four project-town photos with a CSS pan. Both were dropped: the stock footage was
  not American, and the rotating photos were project thumbnails rather than pictures composed to sit
  behind a headline. If ZoneCo footage or photos shot for the hero turn up, they belong here.
